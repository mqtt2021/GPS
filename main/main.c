...
void configure_led(void)
{
    ...
    // gpio_pulldown_en(GPIO_GPS_TRIGGER); // Bật pull-down để giữ mức thấp
...
void app_main(void)
{
    ...
    gpio_set_level(GPIO_GPS_TRIGGER, 1); // Bật GPS
   ...
   // góc hiện tại từ MPU6050
    ...
    if(emergency == 0){
        ...
        while( true ){
            if(strcmp(data.BlueTooth, "ON" ) || strcmp(data.BlueTooth, "OFF" )){
                if(data.Emergency){ 
            ...
            if(alarm_mpu){
                ...
                while(try_connect_gps < 15){
                    ...
                    else
                    {
                        ...
                        gpio_set_level(GPIO_GPS_TRIGGER, 0); // Tắt  GPS
    ...
    else{ // Chế độ bình thường, bật hàng rào
